...
export default function LectureAttendancePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const response = await academyService.getLectureAttendees(lectureId, {
  ...
  }, [lectureId, dateFrom, dateTo]);

  const handleExportPDF = async () => {
    try {
      await academyService.exportAttendeesPDF(lectureId);
      ...
      <div className="mb-6 flex justify-between items-center max-md:flex-col max-md:items-start max-md:gap-4">
        <div className="flex items-center gap-4">
          <Button
            variant="ghost"
            onClick={() => router.back()}
            className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white hover:bg-white/10 transition-colors p-0"
          >
        ...
        <div className="space-y-6">
          {selectedDateStatus === 'not_activated' ? (
            ...
            <>
              ...
              {/* Attendees Table */}
              <DashboardCard title="قائمة الطلاب" icon="users">
                  ...
                  <table className="w-full">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {data.attendees.map((attendee) => (
                        <tr key={attendee.id || attendee.student_id} className="hover:bg-white/5 transition-colors">
