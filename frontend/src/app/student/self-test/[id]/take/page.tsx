...
export default function TakeSelfTestPage() {
  const { user } = useAuth();
  ...
  const loadAttempt = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetchApi(`/student/exams/attempts/${attemptId}/status`) as AttemptData;
      ...
      toast.error(error.message || 'حدث خطأ في تحميل بيانات الاختبار');
      router.push('/student/self-test');
      setLoading(false);
    }
  }, [attemptId, router]);
  ...
  const submitAnswer = useCallback(async (answer: string | null) => {
    ...
    try {
      let response;
      if (answer) {
        response = await fetchApi(`/student/exams/attempts/${attemptId}/answer`, {
          method: 'POST',
          body: JSON.stringify({ answer }),
        }) as any;
      } else {
        response = await fetchApi(`/student/exams/attempts/${attemptId}/skip`, { method: 'POST' }) as any;
      }
  ...
  const terminateExam = useCallback(async (reason: string) => {
    if (!attemptId) return;
    try {
      const response = await fetchApi(`/student/exams/attempts/${attemptId}/terminate`, {
        method: 'POST',
        body: JSON.stringify({ reason }),
      }) as any;
      if (response) {
        setAttemptData(response);
        toast.error('تم إنهاء الاختبار بسبب مخالفة');
      }
        ...
        <div className="fixed inset-0 bg-surface-secondary/95 dark:bg-[#050811]/95 backdrop-blur-2xl flex items-center justify-center z-[1000] p-4 sm:p-5 overflow-y-auto">
          <div className="bg-surface-primary dark:bg-[#101426]/80 rounded-3xl sm:rounded-[48px] p-5 sm:p-10 md:p-14 max-w-[650px] w-full text-center border border-border-theme-primary shadow-2xl relative overflow-hidden my-auto">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-5">
              <Button
                variant="ghost"
                onClick={() => router.push('/student/self-test')}
              ...
              <Button
                variant="primary"
                onClick={() => { setShowWarning(false); loadAttempt(); }}
      ...
      <DashboardLayout role="student" user={user || undefined}>
        <div className="flex flex-col items-center justify-center min-h-[70vh] p-5 text-center">
            <div className="space-y-6 sm:space-y-8">
              ...
              <p className="text-lg sm:text-2xl text-gray-400 font-black tracking-widest animate-pulse uppercase font-mono">جاري تجهيز الاختبار...</p>
            </div>
        </div>
      </DashboardLayout>
    );
        ...
        <div className="max-w-3xl mx-auto py-8 sm:py-16 px-4 sm:px-6">
          <div className="bg-surface-primary dark:bg-[#101426]/60 backdrop-blur-3xl rounded-3xl sm:rounded-[56px] border border-border-theme-primary p-6 sm:p-10 md:p-16 text-center shadow-[0_40px_100px_rgba(0,0,0,0.6)] relative overflow-hidden">
            ...
            <h2 className="text-text-theme-primary text-2xl sm:text-4xl font-black mb-2 sm:mb-4 tracking-tight drop-shadow-md">
              {result?.terminated ? 'توقف الاختبار!' : isPassed ? 'أحسنت! لقد نجحت' : 'نتيجة الاختبار'}
            </h2>
              ...
              <div className="flex flex-col sm:flex-row-reverse gap-3 sm:gap-4">
                ...
                >
                  <span className="relative z-10">
                    {submitting ? 'جاري الحفظ...' : (attemptData?.progress?.current === attemptData?.progress?.total ? 'إنهاء وتسليم الاختبار' : 'تأكيد الإجابة والانتقال التالي')}
                  </span>
                  {selectedAnswer && (
