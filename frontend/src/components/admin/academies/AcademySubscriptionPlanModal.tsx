...
import { updateAcademyPlan } from '@/services/admin/adminService';
import { useSettings } from '@/contexts/SettingsContext';
import { generateInvoicePDF } from '@/utils/generateInvoicePDF';

interface AcademySubscriptionPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
  academy: any;
  onSuccess: () => void;
...
export default function AcademySubscriptionPlanModal({
  isOpen,
  onClose,
  academy,
  onSuccess
}: AcademySubscriptionPlanModalProps) {
  const { settings } = useSettings();
  const pricePerStudent = parseFloat(settings.academyStudentPrice || '20');
  ...
  useEffect(() => {
    if (isOpen && academy) {
      // Initialize with academy's current plan if available
      if (academy.plan_type) {
         if (academy.plan_type === 'trial') {
             setActiveTab('trial');
         } else if (academy.plan_type === 'term') {
             setActiveTab('term');
         } else {
             setActiveTab('custom');
         }
      ...
      if (academy.plan_max_students) {
          setMaxStudents(academy.plan_max_students);
  ...
  const getCurrentPlanFinancials = () => {
    if (!academy?.plan_type || !academy?.plan_expires_at || !academy?.created_at) {
      return null;
    }

    const today = new Date();
    const expiresAt = new Date(academy.plan_expires_at);
    const createdAt = new Date(academy.created_at);
    ...
    // Calculate costs
    const studentsCount = academy.is_unlimited_students ? 0 : (academy.plan_max_students || 50);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateAcademyPlan(academy.id, payload);
  ...
  const handleExportInvoice = async () => {
    ...
    try {
      const hasExistingPlan = academy?.plan_type !== null && academy?.plan_type !== undefined;
      ...
      const invoiceData = {
        ...
        old_plan: hasExistingPlan ? {
          ...
          expires_at: academy.plan_expires_at,
        } : null,
      };

      await generateInvoicePDF(academy.id, invoiceData);
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="text-white font-medium">
                    {academy.plan_type === 'trial' ? 'تجريبية' : 
                     academy.plan_type === 'term' ? 'مدة ثابتة' : 'مخصصة'}
                  </p>
                </div>
                <div>
                  <span className="text-gray-400">عدد الطلاب:</span>
                  <p className="text-white font-medium">
                    {academy.is_unlimited_students ? 'لا نهائي' : academy.plan_max_students || 50}
                  </p>
                </div>
                {academy.plan_expires_at && (
                  <div className="col-span-2">
                    ...
                    <p className="text-white font-medium">
                      {new Date(academy.plan_expires_at).toLocaleDateString('ar-EG')}
                    </p>
                  </div>
                )}
