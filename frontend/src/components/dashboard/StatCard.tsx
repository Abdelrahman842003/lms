    ...
    return (
      <div className="premium-glass p-8 rounded-3xl flex flex-col items-center justify-center text-center group border-border-theme-secondary hover:border-border-theme-primary relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h3 className="text-4xl sm:text-5xl font-black text-text-theme-primary mb-2 tracking-tight">
            {typeof value === 'number' ? (
          ...
          <p className="text-text-theme-secondary font-medium tracking-wide uppercase text-xs sm:text-sm opacity-70 group-hover:opacity-100 transition-opacity">
            {title}
          </p>
        </div>

        {trend && (
          <div className="mt-4 px-3 py-1 rounded-full bg-surface-secondary border border-border-theme-secondary text-xs font-semibold flex items-center gap-1.5">
            ...
            <span className="text-text-theme-secondary opacity-60">{trend.label}</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="premium-glass p-5 rounded-2xl flex items-center gap-4 group border-border-theme-secondary hover:border-border-theme-primary relative overflow-hidden">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-medium text-text-theme-secondary uppercase tracking-wider mb-1 opacity-70 group-hover:opacity-100 transition-opacity">
          {title}
        </p>
        <h3 className="text-2xl font-bold text-text-theme-primary tracking-tight">
          {typeof value === 'number' ? (
            <CountUp end={value} prefix={prefix} suffix={suffix} />
          ) : (
            value
      ...
      {trend && (
        <div className={`px-2 py-1 rounded-lg text-[10px] font-bold ${trend.isPositive ? 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400' : 'bg-rose-500/10 text-rose-600 dark:text-rose-400'} border border-border-theme-secondary`}>
