...
export function StudentHonorBoard() {
  ...
  if (loading) {
    return (
      <div className="flex justify-center py-10 bg-surface-secondary backdrop-blur-xl rounded-3xl border border-border-theme-primary mt-8">
  ...
  if (!selectedTeacher) {
    return (
      <div className="text-center py-8 bg-surface-secondary backdrop-blur-xl rounded-3xl border border-border-theme-primary mt-8">
        <p className="text-text-theme-secondary">اختر مدرساً لعرض لوحة الشرف الخاصة به.</p>
      </div>
    );
  }
      ...
      <div className="relative overflow-hidden premium-glass premium-border rounded-[3rem] p-8 md:p-12 shadow-2xl">
        ...
        <div className="relative flex flex-col sm:flex-row items-center justify-between mb-12 gap-6">
          <div className="flex items-center gap-4">
             ...
             <div className="space-y-1">
                <h3 className="text-2xl md:text-3xl font-black text-text-theme-primary tracking-tight">لوحة الشرف</h3>
                <p className="text-text-theme-muted text-sm font-medium">أفضل الطلاب لدى ({selectedTeacher.teacher_name})</p>
             </div>
          </div>

          <div className="flex items-center gap-2 p-1.5 bg-surface-secondary rounded-2xl border border-border-theme-primary backdrop-blur-xl">
            <button 
              onClick={() => setActiveTab('weekly')} 
              className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-300 
                ${activeTab === 'weekly' ? 'bg-primary text-white shadow-xl shadow-primary/20' : 'text-text-theme-muted hover:text-text-theme-primary hover:bg-surface-hover'}`}
            >
              هذا الأسبوع
            </button>
            <button 
              ...
              className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all duration-300 
                ${activeTab === 'allTime' ? 'bg-primary text-white shadow-xl shadow-primary/20' : 'text-text-theme-muted hover:text-text-theme-primary hover:bg-surface-hover'}`}
            >
              على الإطلاق
            </button>
          </div>
        ...
        {myStats && (
          <div className="mb-12 relative overflow-hidden group">
            ...
            <div className="relative flex flex-col md:flex-row items-center justify-between gap-6 bg-surface-secondary backdrop-blur-xl rounded-[2rem] p-6 md:p-8 border border-border-theme-primary shadow-2xl transition-transform duration-500 group-hover:scale-[1.01]">
              <div className="flex items-center gap-6">
                <div className="relative">
                   <div className="absolute inset-0 bg-primary/30 rounded-full blur-xl animate-pulse"></div>
                   <div className="relative w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-primary to-purple-600 flex items-center justify-center text-2xl md:text-3xl font-black text-white shadow-2xl ring-4 ring-border-theme-primary">
                    #{activeTab === 'weekly' ? myStats.weekly_rank : myStats.rank}
                  </div>
                </div>
                <div className="text-right">
                  <h4 className="text-xl md:text-2xl font-black text-text-theme-primary mb-1">مركزك في لوحة الشرف</h4>
                  <p className="text-primary/60 font-black text-[10px] uppercase tracking-widest">أنت تقوم بعمل رائع، استمر في التقدم!</p>
                </div>
              </div>
              <div className="flex items-center gap-8 px-8 py-4 rounded-2xl bg-surface-tertiary border border-border-theme-secondary">
                <div className="text-center">
                  <div className="text-2xl font-black text-text-theme-primary tabular-nums">{activeTab === 'weekly' ? myStats.weekly_points : myStats.total_points}</div>
                  <div className="text-[10px] font-black text-text-theme-muted uppercase tracking-widest">نقطة</div>
                </div>
                <div className="w-px h-10 bg-border-theme-secondary"></div>
                <div className="text-center">
                  <div className="text-2xl font-black text-text-theme-primary tabular-nums">{myStats.attendance_streak}</div>
                  <div className="text-[10px] font-black text-text-theme-muted uppercase tracking-widest">يوم حضور</div>
                </div>
              </div>
            </div>
          </div>
        ...
        {top3.length > 0 ? (
              ...
              return (
                <div key={config.position} className={`${config.order} flex flex-col items-center transition-all duration-700 hover:-translate-y-3 group/podium`}>
                  ...
                  <div className="relative mb-6">
                    ...
                    <div className={`relative w-20 h-20 md:w-32 md:h-32 rounded-full border-4 ${config.borderColor} overflow-hidden shadow-2xl bg-surface-secondary ring-8 ring-border-theme-secondary`}>
                  ...
                  <div className="text-center space-y-3 mb-6">
                     <h3 className="text-text-theme-primary font-black text-sm md:text-lg max-w-[120px] md:max-w-[180px] truncate drop-shadow-lg">{entry.student.name}</h3>
                  ...
                  <div className={`w-24 md:w-44 ${config.height} ${config.bgGradient} border-t-4 ${config.borderColor} rounded-t-[2.5rem] flex items-center justify-center premium-glass shadow-[0_-20px_50px_rgba(0,0,0,0.3)] relative overflow-hidden group/box`}>
                    <div className="absolute inset-0 bg-surface-primary/10 transition-colors group-hover/podium:bg-surface-primary/30"></div>
          ...
          <div className="text-center py-20 bg-surface-secondary rounded-[2rem] border border-border-theme-secondary">
            <div className="w-16 h-16 rounded-2xl bg-surface-tertiary border border-border-theme-primary flex items-center justify-center text-text-theme-muted/40 text-2xl mx-auto mb-4">
               <Icon name="users-slash" />
            </div>
            <p className="text-text-theme-muted font-black uppercase tracking-widest text-xs">لا يوجد بيانات للوحة الشرف حالياً</p>
          </div>
        )}
      </div>
    </div>
