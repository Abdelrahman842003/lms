...
export const LectureCard: React.FC<LectureCardProps> = ({
  ...
  return (
    <div className={cn(
      "group relative flex flex-col transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(0,0,0,0.4)]",
      isMenuOpen ? 'z-[100]' : ''
    )}>
      {/* Background & Clipping Layer */}
      <div className={cn(
        "absolute inset-0 rounded-[2.5rem] premium-glass premium-border overflow-hidden",
      ...
      {/* Real Content (No overflow-hidden here) */}
      <div className="relative z-10 flex flex-col p-7 h-full">
         {/* Top Section */}
         <div className="flex justify-between items-start mb-6">
            <div className="flex flex-col gap-2">
               <div className="flex items-center gap-2">
                  <span className={cn("w-2 h-2 rounded-full animate-pulse", status.variant === 'success' ? 'bg-success shadow-[0_0_8px_var(--success)]' : status.variant === 'warning' ? 'bg-warning' : 'bg-gray-400')}></span>
                  <span className="text-[10px] font-black uppercase tracking-widest text-white/40">{status.label}</span>
               </div>
               {isActive && timeLeft && (
            ...
            <div className="relative">
               ...
               {isMenuOpen && (
                 <div className="actions-menu show actions-menu-card !bg-[#0f1121] !opacity-100 !visible !translate-y-0 !z-[999] !shadow-2xl">
                    <button className="actions-menu-item ux-w-full" onClick={(e) => { e.stopPropagation(); onViewAttendees(); }}>
                       <Icon name="clipboard-list" size="sm" /> <span>سجل الحضور</span>
                    </button>
                    {isActive && (
                       <button className="actions-menu-item ux-w-full" onClick={(e) => { e.stopPropagation(); onManualAttendance(); }}>
                          <Icon name="user-check" size="sm" /> <span>حضور يدوي</span>
                       </button>
                    )}
                    <div className="h-px bg-white/5 my-2"></div>
                    <button className="actions-menu-item ux-w-full" onClick={(e) => { e.stopPropagation(); onEdit(); }}>
                       <Icon name="edit" size="sm" /> <span>تعديل المحاضرة</span>
                    </button>
                    <button className="actions-menu-item ux-w-full" onClick={(e) => { e.stopPropagation(); onCopy(); }}>
                       <Icon name="copy" size="sm" /> <span>نسخ البيانات</span>
                    </button>
                    <div className="h-px bg-white/5 my-2"></div>
                    <button className="actions-menu-item danger ux-w-full" onClick={(e) => { e.stopPropagation(); onDelete(); }}>
                       <Icon name="trash" size="sm" /> <span>حذف المحاضرة</span>
                    </button>
                 </div>
               )}
            </div>
         ...
         {/* Main Content */}
         <div className="mb-6">
            <h3 className="text-2xl font-black text-white mb-2 leading-tight group-hover:text-primary transition-colors">{lecture.title}</h3>
         ...
         <div className="grid grid-cols-2 gap-4 mb-8">
            <div className="flex items-center gap-3 bg-white/[0.03] p-3 rounded-2xl border border-white/5 transition-all hover:bg-white/5">
               <div className="w-8 h-8 rounded-xl bg-primary/10 flex items-center justify-center"><Icon name="calendar" size="sm" className="text-primary" /></div>
               <div className="flex flex-col overflow-hidden">
                  <span className="text-[10px] text-gray-light/40 font-bold uppercase">التاريخ</span>
                  <span className="text-xs text-white font-bold truncate">{lecture.is_recurring && lecture.recurrence_days?.length ? 'يومي' : lecture.date}</span>
               </div>
            </div>
            <div className="flex items-center gap-3 bg-white/[0.03] p-3 rounded-2xl border border-white/5 transition-all hover:bg-white/5">
               <div className="w-8 h-8 rounded-xl bg-secondary/10 flex items-center justify-center"><Icon name="clock" size="sm" className="text-secondary" /></div>
            ...
            <div className="flex items-center gap-3 bg-white/[0.03] p-3 rounded-2xl border border-white/5 transition-all hover:bg-white/5">
               <div className="w-8 h-8 rounded-xl bg-purple-500/10 flex items-center justify-center"><Icon name="users" size="sm" className="text-purple-500" /></div>
               <div className="flex flex-col overflow-hidden">
            ...
            <div className="flex items-center gap-3 bg-white/[0.03] p-3 rounded-2xl border border-white/5 transition-all hover:bg-white/5">
               <div className="w-8 h-8 rounded-xl bg-warning/10 flex items-center justify-center"><Icon name="user-check" size="sm" className="text-warning" /></div>
               <div className="flex flex-col">
         ...
         {lecture.status !== 'منتهية' && (
            <div className="mt-auto pt-6 border-t border-white/5">
               {!isActive ? (
                  <Button onClick={onActivate} className="w-full h-12 rounded-2xl bg-gradient-to-r from-primary to-secondary hover:shadow-[0_8px_20px_rgba(66,99,235,0.4)] text-white font-black uppercase tracking-widest text-xs gap-2 transition-all border-none">
                     <Icon name="power-off" /> <span>تفعيل المحاضرة الآن</span>
                  </Button>
               ) : (
                  <div className="flex items-center gap-2">
                     <Button onClick={onScan} className="flex-1 h-12 rounded-2xl bg-white/5 border border-white/10 hover:bg-white/10 text-white font-bold text-xs gap-2 transition-all">
                        <Icon name="camera" /> <span>مسح</span>
                     </Button>
                     <Button onClick={onQRCode} className="flex-1 h-12 rounded-2xl bg-primary/10 border border-primary/20 hover:bg-primary/20 text-primary font-bold text-xs gap-2 transition-all">
                        <Icon name="qrcode" /> <span>QR</span>
                     </Button>
                     <Button onClick={onEnd} className="w-12 h-12 rounded-2xl bg-red-500/10 border border-red-500/20 hover:bg-red-500/20 text-red-500 flex items-center justify-center transition-all">
                        <Icon name="stop" />
                     </Button>
                  </div>
               )}
