  ...
  return (
    <>
      <div className={`fixed top-0 left-0 right-0 z-[100] flex justify-center transition-all duration-300 ${isScrolled ? 'pt-4' : 'pt-6 md:pt-8'}`}>
        <nav 
          className={`w-[92%] md:w-[85%] max-w-[1200px] premium-glass border border-border-theme-secondary rounded-full flex items-center justify-between px-4 md:px-8 py-3 transition-all duration-300 shadow-[0_8px_32px_0_rgba(0,0,0,0.08)] ${isScrolled ? 'bg-surface-secondary/80 border-border-theme-primary' : ''}`}
        >
          {/* Logo area */}
          <Link href="/" className="flex items-center gap-2 md:gap-3 group cursor-pointer">
            ...
            <span className="text-text-theme-primary font-bold tracking-wide text-base md:text-xl">{projectName}</span>
          </Link>
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.path}
                  className={`text-[0.9rem] font-medium transition-all duration-300 hover:text-[#3249A9] ${isActive ? 'text-[#3249A9]' : 'text-text-theme-secondary'}`}
                >
                  {link.name}
                </Link>
              );
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <button 
              ...
              className="lg:hidden w-9 h-9 flex flex-col justify-center items-center gap-1.5 bg-surface-secondary rounded-full border border-border-theme-primary"
            >
              <span className={`w-4 h-0.5 bg-text-theme-primary rounded transition-all duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
              <span className={`w-4 h-0.5 bg-text-theme-primary rounded transition-all duration-300 ${isMobileMenuOpen ? 'opacity-0' : ''}`}></span>
              <span className={`w-4 h-0.5 bg-text-theme-primary rounded transition-all duration-300 ${isMobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
            </button>
          </div>
        </nav>
      ...
      {/* Mobile Menu Overlay */}
      <div 
        className={`fixed inset-0 z-[90] bg-surface-primary/95 backdrop-blur-2xl transition-all duration-500 lg:hidden flex flex-col items-center justify-center gap-8 ${isMobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
      >
          {navLinks.map((link) => {
            const isActive = pathname === link.path;
            return (
              <Link
                key={link.name}
                href={link.path}
                className={`text-2xl font-bold transition-all duration-300 ${isActive ? 'text-[#3249A9]' : 'text-text-theme-primary hover:text-[#3249A9]'}`}
              >
                {link.name}
              </Link>
            );
