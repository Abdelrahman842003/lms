  ...
  useEffect(() => {
    ...
    try {
      ...
      if (storedUser) {
        ...
        document.cookie =
          "auth_state=true; path=/; max-age=2592000; SameSite=Lax";
        ...
        if (userType) {
          ...
          try {
            ...
            const userData: User = {
              ...
              parent_phone: response.parent_phone || response.user.parent_phone, // For parent, it's at root level
            ...
            if (userType === "parent" && response.children) {
              ...
              const currentSelected = localStorage.getItem("selectedChild");
              if (currentSelected) {
                const parsedCurrent = JSON.parse(currentSelected);
                const stillExists = response.children.find(
                  (c: any) => c.id === parsedCurrent.id
                );
                if (stillExists) {
                  setSelectedChild(stillExists);
                  localStorage.setItem(
                    "selectedChild",
                    JSON.stringify(stillExists)
                  );
                } else if (response.children.length > 0) {
                  setSelectedChild(response.children[0]);
                  localStorage.setItem(
                    "selectedChild",
                    JSON.stringify(response.children[0])
                  );
                }
              }
            }
            ...
            if (
              ...
              const activeTeacher = response.teachers.find(
                (t: any) => t.status === "active" && !t.is_suspended
              );
              const graceTeacher = response.teachers.find(
              ...
              if (currentSelected) {
                const parsedCurrent = JSON.parse(currentSelected);
                const updatedCurrent = response.teachers.find(
                ...
                if (
                  ...
                  (updatedCurrent.status === "active" ||
                    updatedCurrent.status === "grace_period") &&
            ...
            if (
              apiError.status === 401 ||
              apiError.message?.toLowerCase().includes("unauthenticated")
            ) {
              console.error(
                "AuthContext: User unauthenticated (401), clearing session. Error details:",
                apiError
              );
              setUser(null);
              setSelectedTeacher(null);
    ...
    const handleUnauthorized = () => {
      ...
      document.cookie =
        "auth_state=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
      document.cookie =
        "user_role=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
      document.cookie =
        "laravel_session=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
      document.cookie =
    ...
    return () => {
      ...
      window.removeEventListener(
        "auth:teacher_suspended",
        handleTeacherSuspended
      );
    };
  }, [router]);
  ...
  const login = async (
    ...
    try {
      ...
      if (
        ...
        localStorage.setItem(
          "studentTeachers",
          JSON.stringify(response.teachers)
        );
      ...
      document.cookie =
        "auth_state=true; path=/; max-age=2592000; SameSite=Lax"; // 30 days
  ...
  const enableNotifications = async () => {
    try {
      ...
      if (token) {
        ...
        const { storeDeviceToken } = await import(
        ...
        console.log(
          "[FCM] No token received - user may have denied permission"
        );
      }
    } catch (error) {
