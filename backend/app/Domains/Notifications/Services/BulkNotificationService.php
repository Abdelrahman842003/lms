...
class BulkNotificationService
{
    ...
    public function send(Builder $query, string $title, string $message, array $data = [])
    {
        ...
        if (! $sendInternal && ! $sendExternal) {
            Log::info('Bulk notification skipped because all channels are disabled.');
            return;
        }

        // Chunk the query to avoid memory issues
        $query->chunk($batchSize, function ($users) use ($title, $message, $data, $batchSize, $sendInternal, $sendExternal) {
            $users = $this->notificationSettings->filterRecipients($users);
            ...
            foreach ($users as $user) {
                ...
                if ($sendExternal && method_exists($user, 'deviceTokens')) {
            ...
            if ($sendInternal && !empty($notificationsData)) {
                DB::table('notifications')->insert($notificationsData);
            }

            // Dispatch FCM Job for offline/background users
            if ($sendExternal && !empty($tokens)) {
                foreach (array_chunk($tokens, $batchSize) as $tokenChunk) {
