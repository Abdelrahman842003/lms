...
class TeacherService
{
    ...
    public function getSubscriptionForMonth(string $teacherId, string $month): \App\Domains\Auth\Models\TeacherSubscription
    {
        $teacher = Teacher::findOrFail($teacherId);
        ...
        $subscription = $teacher->subscriptions()->firstOrCreate(
            ['month' => $monthDate],
            [
                'student_count' => \App\Domains\Subscriptions\Models\PaymentLog::where('teacher_id', $teacherId)
        ...
        if ($subscription->status === 'pending') {
            ...
            $billableMonths = \App\Domains\Subscriptions\Models\PaymentLog::where('teacher_id', $teacherId)
                ->where('status', 'confirmed')
    ...
    public function paySubscription(string $teacherId, string $month, float $amount): \App\Domains\Auth\Models\TeacherSubscription
    {
        ...
        if ($amount > 0) {
            ...
            if (!$subscription->payment_key) {
                $subscription->payment_key = \App\Domains\Auth\Models\TeacherSubscription::generatePaymentKey();
