...
class LectureObserver
{
    ...
    public function created(Lecture $lecture): void
    {
        ...
        if ($lecture->is_recurring) {
            ...
            if ($lecture->start_time->isFuture()) {
                ...
                \App\Jobs\ProcessLectureStart::dispatch($lecture);
    ...
    private function findNextOccurrence(Lecture $lecture, \Carbon\Carbon $from): ?\Carbon\Carbon
    {
        ...
        if (in_array($today->format('l'), $lecture->recurrence_days)) {
            $todayStart = \Carbon\Carbon::parse($today->format('Y-m-d') . ' ' . $lecture->recurrence_time, 'Africa/Cairo')
                ->setTimezone('UTC');
            
            if ($todayStart->isAfter($from)) {
                return $todayStart;
            }
        }
        ...
        // Check next 7 days
        for ($i = 1; $i <= 7; $i++) {
            $date = $from->copy()->addDays($i)->setTimezone('Africa/Cairo');
            if (in_array($date->format('l'), $lecture->recurrence_days)) {
                return \Carbon\Carbon::parse($date->format('Y-m-d') . ' ' . $lecture->recurrence_time, 'Africa/Cairo')
                    ->setTimezone('UTC');
            }
        }
