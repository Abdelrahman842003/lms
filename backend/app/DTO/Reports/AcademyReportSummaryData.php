...
final readonly class AcademyReportSummaryData
{
    ...
    public function toArray(): array
    {
        return [
            ...
            'subscription_fee' => $this->subscriptionFee, // السعر المدفوع للمنصة
     ...
     * Formula: plan_max_students × duration_months × price_per_student
     *
     * @param int $maxStudents Maximum students in plan
     * @param int $durationMonths Duration in months
     * @param float $pricePerStudent Price per student (40 EGP for academies)
     */
    public static function calculateSubscriptionFee(
        int $maxStudents,
        int $durationMonths,
        float $pricePerStudent
    ): float {
        return $maxStudents * $durationMonths * $pricePerStudent;
    }
}
